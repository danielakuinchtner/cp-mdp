...
for num_actions in prange(len(actions)):
    if len(actions) < 7:
...
final_limits = []
for num_dim in prange(len(shape)):
...
obstacles = []
for o in prange(len(obs)):
    obstacles.append(obs[o].tolist())

terminals = []
for o in prange(len(term)):
...
for a1 in prange(len(STPM[0])):
    for a2 in prange(len(STPM[1])):
        if a1 == a2:
